...
exports.getStreamToken = functionsV1
    ...
    try {
        const { StreamClient } = await Promise.resolve().then(() => require('@stream-io/node-sdk'));
...
exports.handleVoiceBooking = (0, https_1.onRequest)({ cors: true, secrets: ["GOOGLE_GENERATIVE_AI_API_KEY"] }, async (req, res) => {
    ...
    const { intent, phone, uid, category, preferred_time, user_name, notes, } = req.body;
    ...
    try {
        ...
        switch (intent) {
            ...
            case "check_availability": {
                ...
                const matchedSpecialists = specialistsSnap.docs
                    ...
                    .filter((s) => {
                    const spec = (s.specialty || s.primaryRole || s.skills || "").toLowerCase();
                ...
                slot1.setHours(Math.max(9, Math.min(18, now.getHours() + 1)), 0, 0, 0);
                ...
                const existingAppts = await db
                    ...
                    .get();
                const bookedTimes = existingAppts.docs.map((d) => d.data().timestamp);
                const slots = [slot1.toISOString(), slot2.toISOString()].filter((s) => !bookedTimes.includes(s));
            ...
            case "book_appointment": {
                ...
                if (!resolvedUid) {
                    res.status(200).json({
                        booked: false,
                        message: "I couldn't verify your identity. Could you provide your registered phone number?",
                    });
                    return;
                }
                ...
                if (!conflictSnap.empty) {
                    res.status(200).json({
                        booked: false,
                        conflict: true,
                        message: "That time slot is already taken. Let me find you another available time.",
                    });
                    return;
                }
            ...
            case "emergency": {
                ...
                res.status(200).json({
                    ...
                    message: "I am an AI and cannot provide emergency services. Please contact your local emergency number immediately or visit the nearest crisis center. Your safety is paramount. I have flagged this for our human team to follow up with you.",
                });
                return;
            }
            // ────────────────────────────────────────────────────────────────
            ...
            case "escalate": {
                ...
                res.status(200).json({
                    ...
                    message: "I've created a support ticket and notified our care team. A social worker will reach out to you shortly. Is there anything else I can help you with in the meantime?",
                });
                return;
            }
            // ────────────────────────────────────────────────────────────────
        ...
        res.status(500).json({
            ...
            message: "I encountered a technical issue. Please try again or contact support directly.",
            details: error.message,
        });
    }
});
...
exports.seedAgentBehaviorConfig = functionsV1.https.onCall(async (_data, context) => {
    ...
    if (callerRole !== "admin") {
        throw new functionsV1.https.HttpsError("permission-denied", "Only admins can configure agent behavior.");
    }
    const config = {
        // ── Turn-Taking ──
        ...
        double_talk_apology: "Sorry, please continue, I didn't mean to cut you off.",
        // ── Wait Policy ──
        wait_policy: "If the user stops talking, do not interrupt. Wait for a clear pause. " +
        ...
        // ── Listening Protocol ──
        listening_protocol: "You are a 'Deep Listener.' If the user pauses, do not interpret it " +
        ...
        // ── Emergency Protocol ──
        emergency_response: "I am an AI and cannot provide emergency services. Please contact " +
...
exports.agoraToken = (0, https_1.onRequest)({ cors: true }, async (req, res) => {
    ...
    try {
        ...
        const token = RtcTokenBuilder.buildTokenWithUid(appId, appCertificate, channelName, uid || 0, rtcRole, expirationTimestamp, expirationTimestamp);
